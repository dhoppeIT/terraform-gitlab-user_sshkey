...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_expires_at"></a> [expires\_at](#input\_expires\_at) | The expiration date of the SSH key in ISO 8601 format (YYYY-MM-DDTHH:MM:SSZ) | `string` | `null` | no |
| <a name="input_key"></a> [key](#input\_key) | The ssh key. The SSH key comment (trailing part) is optional and ignored for diffing, because GitLab overrides it with the username and GitLab hostname | `string` | n/a | yes |
| <a name="input_title"></a> [title](#input\_title) | The title of the ssh key | `string` | n/a | yes |
| <a name="input_user_id"></a> [user\_id](#input\_user\_id) | The ID or username of the user. If this field is omitted, this resource manages a SSH key for the current user. Otherwise, this resource manages a SSH key for the specified user, and an admin token is required | `number` | `null` | no |
